...
def is_node_func(coords):
    return (coords[0] - coords[1]//2 > -1) and (2*coords[0] - coords[1] < 2*(n2-1))
...
#SpringsView(T,XI,E, process,((-1,6),(-1,5)),time_text_coords=(0.,-0.5),"example4_skewed_homo_disp.mp4",20) #to save the movie in a file
SpringsView(T,XI,E, process,((-1,6),(-1,5)),time_text_coords=(0.,-0.5))


XI_leapfrog=np.tile(np.expand_dims(xi0, axis=1),(1,T_leapfrog.shape[0]))
SpringsView(T_leapfrog,XI_leapfrog, E_leapfrog, process, ((-1,6),(-1,5)),time_text_coords=(0.,-0.5))
plt.show()